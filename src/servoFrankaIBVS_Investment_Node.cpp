...
// --- Optional ROS 2 wrapper -------------------------------------------------
// This file was originally a standalone ViSP/Franka application (plain main()).
// For workflows that require the application to be a ROS 2 node (e.g., discovery
// by an MCP), we conditionally compile a thin rclcpp::Node wrapper when ROS 2
// headers are available.
//
// Build in a ROS 2 ament_cmake package that depends on rclcpp + std_srvs.
#if __has_include(<rclcpp/rclcpp.hpp>)
#  define SERVOFRANKA_HAVE_ROS2 1
...
#  include <thread>
#else
#  define SERVOFRANKA_HAVE_ROS2 0
#endif
...
#if SERVOFRANKA_HAVE_ROS2
// Global flags set by the ROS 2 wrapper node (if used).
static std::atomic_bool *g_stop_requested = nullptr;
static std::atomic_bool *g_enabled = nullptr;

static inline bool stopRequested()
{
  return (g_stop_requested != nullptr) && g_stop_requested->load();
}

static inline bool rosEnabled()
{
  // If the wrapper isn't used, default to enabled.
  return (g_enabled == nullptr) || g_enabled->load();
}
#endif
...
// Original application entrypoint (standalone). When ROS 2 is available,
// a thin Node wrapper calls this function from a worker thread.
static int servoFrankaIBVS_app(int argc, char **argv)
{
  ...
  try {
    ...
    while (!has_converged && !final_quit) {
      ...
      if (!rosEnabled()) {
        // Gate motion when disabled via ROS.
        send_velocities = false;
      }
#endif
...
#if SERVOFRANKA_HAVE_ROS2
// ROS 2 wrapper: expose this application as a discoverable rclcpp node.
class ServoFrankaIBVSNode : public rclcpp::Node
{
public:
  ServoFrankaIBVSNode(int argc, char **argv)
    : rclcpp::Node("servo_franka_ibvs"), argc_(argc), argv_(argv)
  {
    // Default to disabled so nothing moves until explicitly enabled.
    enabled_.store(false);
    stop_requested_.store(false);

    // Make the legacy loop aware of ROS-controlled enable/stop.
    g_enabled = &enabled_;
    ...
    enable_srv_ = this->create_service<std_srvs::srv::SetBool>(
      ...
      [this](const std_srvs::srv::SetBool::Request::SharedPtr req,
        ...
        RCLCPP_INFO(this->get_logger(), "Servo %s", req->data ? "ENABLED" : "DISABLED");
      });

    // Keep parameter and internal flag in sync.
    param_timer_ = this->create_wall_timer(
      std::chrono::milliseconds(200),
      [this]() {
        const bool p = this->get_parameter("enabled").as_bool();
        enabled_.store(p);
      });

    worker_ = std::thread([this]() {
      RCLCPP_INFO(this->get_logger(), "Starting ViSP/Franka servo worker thread...");
      const int ret = servoFrankaIBVS_app(argc_, argv_);
      RCLCPP_INFO(this->get_logger(), "Servo worker exited with code %d", ret);
      // Request shutdown once the worker ends (e.g., user quits via display).
      stop_requested_.store(true);
      rclcpp::shutdown();
    });
  }

  ~ServoFrankaIBVSNode() override
  {
    stop_requested_.store(true);
    enabled_.store(false);
    if (worker_.joinable()) {
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<ServoFrankaIBVSNode>(argc, argv);
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
...
int main(int argc, char **argv)
{
  return servoFrankaIBVS_app(argc, argv);
}
#endif
#else
int main()
